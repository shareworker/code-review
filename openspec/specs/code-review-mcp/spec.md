# code-review-mcp Specification

## Purpose

`@shareworker/code-review-mcp` 是一个 stdio 传输的 MCP server，把 code review 的确定性工程层（文件选择、智能打包、规则匹配、评论定位、评论反思）暴露为 5 个工具，供 host agent（Claude Code、Codex、Devin）在自己的 LLM 循环里调用。server 不调用任何 LLM；所有 LLM 推理由 host 完成。
